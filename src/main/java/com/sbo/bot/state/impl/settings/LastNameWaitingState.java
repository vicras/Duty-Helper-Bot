...
@Slf4j
@Component
public class LastNameWaitingState extends State {

    private final LastNameHandler lastNameHandler;
    private final BackSettingsHandler backSettingsHandler;

    public LastNameWaitingState(CurrentPersonProvider personProvider, ApplicationEventPublisher publisher, PersonService personService, LastNameHandler lastNameHandler, BackSettingsHandler backSettingsHandler) {
    ...
    protected RequestOperator getRequestOperator(Update update) {
        ...
        SendMessage message = InlineMessageBuilder.builder(personProvider.getCurrentPerson())
                .header("Send me your new last name")
                .row()
                .button("Back", BACK)
                .build();
